...
ReleaseNotes = @'
- Fix: License catalog download (`Get-LicenseSourceData`) now falls back to the existing stale cache when GitHub is unreachable after all retry attempts, instead of failing outright; a warning reports the fallback and its cache age.
